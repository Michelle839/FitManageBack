...
async function cambiarEstado(req, res) {
    try {
        const notificacion = await NotificacionUsuarioService.cambiarEstado(req.params.id_notificacion);
        res.status(200).json(notificacion);
    } catch (error) {
        res.status(error.statusCode || 500).json({
            message: error.message || "Error interno del servidor",
...
export default {crearNotificacion, obtenerNotificaciones, cambiarEstado};